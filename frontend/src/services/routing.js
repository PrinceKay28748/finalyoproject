...
// services/routing.js
// Dijkstra's algorithm — uses contextual edge costs from costFunction.js
// instead of raw distance so routes reflect surface, lighting, incline etc.
...
export function findShortestPath(graph, startNodeId, endNodeId, profileKey = "standard") {
  ...
  while (heap.size > 0) {
    ...
    for (const neighbor of neighbors) {
      ...
      const edgeCost = calculateEdgeCost(edge, profile, context.timePeriod, context.vehicleRestricted);
